...
class Guide(Node):
    def __init__(self):
        ...
        self.route_sub = self.create_subscription(
            Route, "route", self.route_callback, default_qos_profile
        )
    ...
    def waypoint_callback(self, msg):
        ...
        self.get_logger().info(
            f"Received target waypoint: lat = {msg.latitude}, lon = {msg.longitude}"
        )
    ...
    def route_callback(self, msg):
        ...
        self.route = (
            msg.waypoints
        )  # Assuming msg.waypoints is a list of Waypoint messages
    ...
    def calculate_track_setpoints(self):
        ...
        if not self.route_received or not self.current_position:
            self.get_logger().warning(
                "Track Mode: Route or current position not available."
            )
            return None, None
        ...
        if self.current_waypoint_index >= len(self.route):
            self.get_logger().info(
                "Track Mode: All waypoints reached. Switching to position hold."
            )
        ...
        if distance <= self.waypoint_tolerance:
            self.get_logger().info(
                f"Track Mode: Waypoint {self.current_waypoint_index + 1} reached."
            )
            self.current_waypoint_index += 1  # Move to the next waypoint
            if self.current_waypoint_index >= len(self.route):
                self.get_logger().info(
                    "Track Mode: Final waypoint reached. Holding position."
                )
                self.track_completed = True
                return None, None
            else:
        ...
        if self.current_mode == 0:  # Standby
            ...
            if self.track_completed:
                ...
                if desired_heading is not None and desired_speed is not None:
                    self.publish_setpoints(desired_heading, desired_speed)
                    self.get_logger().info(
                        f"Track Mode: Publishing setpoints - Heading: {desired_heading:.2f} deg, Speed: {desired_speed:.2f} m/s"
                    )
                else:
                    self.get_logger().warning(
                        "Track Mode: Unable to calculate setpoints due to missing data."
                    )
