...
class Allokering(Node):
    ...
    def __init__(self):
        ...
        vessel_config_path = os.path.join(
            yaml_package_path,
            self.get_parameter("vessel_config_file").get_parameter_value().string_value,
        )
        ...
        self.propulsion_config_path = os.path.join(
            yaml_package_path,
        ...
        self.tau_sub = self.create_subscription(
            Tau, "tau_control", self.tau_callback, default_qos_profile
        )

        self.thruster1_pub = self.create_publisher(
            ThrusterSignals, "thruster_1_setpoints", default_qos_profile
        )
        self.thruster2_pub = self.create_publisher(
            ThrusterSignals, "thruster_2_setpoints", default_qos_profile
        )
        self.tau_max_pub = self.create_publisher(Tau, "tau_max", default_qos_profile)

        # Add subscriber for stop command
        self.stop_command_active = False
        self.stop_command_sub = self.create_subscription(
            Bool, "stop_command", self.stop_command_callback, default_qos_profile
        )
        ...
        self.propellerDiameter_1 = self.propuslion_config["main_propulsion_1"][
            "propeller"
        ]["diameter"]
        self.Kt0_1 = self.propuslion_config["main_propulsion_1"]["propeller"]["Kt0"]
        self.max_rps_1 = (
            self.propuslion_config["main_propulsion_1"]["propeller"]["max_rpm"] / 60
        )
        self.min_rps_1 = (
            self.propuslion_config["main_propulsion_1"]["propeller"]["min_rpm"] / 60
        )
        self.position_1 = self.propuslion_config["main_propulsion_1"][
            "position"
        ]  # [-1,-0.5,0.3]
        ...
        self.propellerDiameter_2 = self.propuslion_config["main_propulsion_2"][
            "propeller"
        ]["diameter"]
        self.Kt0_2 = self.propuslion_config["main_propulsion_2"]["propeller"]["Kt0"]
        self.max_rps_2 = (
            self.propuslion_config["main_propulsion_2"]["propeller"]["max_rpm"] / 60
        )
        self.min_rps_2 = (
            self.propuslion_config["main_propulsion_2"]["propeller"]["min_rpm"] / 60
        )
        self.position_2 = self.propuslion_config["main_propulsion_2"][
            "position"
        ]  # [-1,0.5,0.3]
    ...
    def stop_command_callback(self, msg: Bool):
        """Callback to handle stop command."""
        self.stop_command_active = msg.data
        if self.stop_command_active:
            self.get_logger().info("Stop command received. Thrusters will be stopped.")
    ...
    def rps(self, x, y, KT0, propellerDiameter):
        if y != 0.0:
            ...
            T0 = x
        return np.sign(T0) * np.sqrt(
            (2 * np.abs(T0)) / (self.rho * KT0 * (propellerDiameter**4))
        )

    def force(self, lim, KT0, propellerDiameter):  # lim gitt i rps; rpm/60

        return (
            0.5 * self.rho * KT0 * (propellerDiameter**4) * abs(lim) * lim
        )  # lim**2 * (self.rho * KT0 * (propellerDiameter ** 4))/ 2

    def step_allokering(self):
        """Main allocation step."""
        if self.stop_command_active:
            # If stop command is active, override normal allocation
            thruster1_message = ThrusterSignals()
            thruster1_message.thruster_id = self.id_1
            thruster1_message.rps = 0.0
            thruster1_message.pitch = 0.0
            thruster1_message.azimuth_deg = 0.0
            thruster1_message.active = False  # Deactivate thruster
            thruster1_message.error = False

            thruster2_message = ThrusterSignals()
            thruster2_message.thruster_id = self.id_2
            thruster2_message.rps = 0.0
            thruster2_message.pitch = 0.0
            thruster2_message.azimuth_deg = 0.0
            thruster2_message.active = False  # Deactivate thruster
            thruster2_message.error = False

            self.get_logger().info("Stop command active: Thrusters set to zero.")

        # Normal allocation logic follows if stop_command_active is False
        else:
            We = np.eye(4)
            ...
            Twt = (
                np.linalg.inv(We) @ Te.T @ np.linalg.inv(Te @ np.linalg.inv(We) @ Te.T)
            )
            fe = Twt @ self.tau
        ...
        if self.force(
            self.max_rps_1, self.Kt0_1, self.propellerDiameter_1
        ) <= self.force(self.max_rps_2, self.Kt0_2, self.propellerDiameter_2):
        ...
        tau_max_message = Tau()
        tau_max_message.surge_x = 2 * max_surge
