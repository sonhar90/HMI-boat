...
from ngc_interfaces.msg import ThrusterSignals, SystemMode, GNSS, HeadingDevice, OtterStatus
...
from ngc_utils.nmea_utils import create_gga_message, create_vtg_message, create_hdt_message, create_rot_message, create_sog_cog_vtg_message
...
class otter_connector():
    def __init__(self, ros_node):
        ...
        self.verbose = self.node.simulation_config['otter_interface']['connection_debug_messages']
    ...
    def check_connection(self):
        if self.verbose:
            self.node.get_logger().info(f"Connection status is {self.connection_status}")
    ...
    def read_message(self, timeout = 10):
        ...
        if ready[0]:
            try:
                ...
                if self.verbose:
                    self.node.get_logger().info("Error in recieving message. This is going to fast")
            ...
            if checksum(gps_message[1:-3]) != gps_message[-2:].lower():
                ...
                if gps_message[2] != "" and gps_message[4] != "": 
                    ...
                    try:
                        ...
                        if self.verbose:
                            self.node.get_logger().info("Could not get GPS coordintes. Check GPS coverage!")
                        lat = 0
                        lon = 0
    ...
    def set_manual_control_mode(self, force_x, force_y, torque_z):
        ...
        if not self.check_connection():
            ...
            self.node.get_logger().warning("Connection lost. Attempting to reconnect...")
            connected = self.establish_connection(self.node.simulation_config['otter_interface']['ip'], self.node.simulation_config['otter_interface']['port'])

            if not connected:
                self.node.get_logger().error("Reconnection failed. Skipping command send.")
                return False
        ...
        if self.verbose:
            self.node.get_logger().info(f"Sending manual control message: {message_to_send}")
...
class OtterUSVNode(Node):
    def __init__(self):
        ...
        if self.simulation:
            ...
            self.otter_status_pub = self.create_publisher(OtterStatus, 'otter_status', default_qos_profile) 

            package_share_directory = get_package_share_directory('ngc_otter_interface')
            csv_file_path           = os.path.join(package_share_directory, 'resource', 'parsed_data.csv')
    ...
    def publish_measurements(self):
        ...
        if self.simulation == False:
            ...
            if self.otter.check_connection() == False:
                ...
                connected = self.otter.establish_connection(self.simulation_config['otter_interface']['ip'], self.simulation_config['otter_interface']['port'])
            ...
            if self.simulation_config['otter_interface']['log_measurements_to_csv']:
                ...
                self.csv_writer.writerow([time.time(), self.otter.current_position[0], self.otter.current_position[1], self.otter.current_orientation[2], self.otter.current_rotational_velocities[2], self.otter.current_speed, 
                                          self.otter.current_course_over_ground, rpm_port_setpoint, self.otter.rpm_port, rpm_strb_setpoint, self.otter.rpm_strb, self.otter.current_mode, self.otter.current_fuel_capacity, self.fx, self.fz])
                self.log_file.flush()
            ...
            if self.simulation_config['otter_interface']['publish_nmea_to_localhost']:
                gga_message = create_gga_message(self.otter.current_position[0], self.otter.current_position[1])
                vtg_message = create_sog_cog_vtg_message(self.otter.current_speed, self.otter.current_course_over_ground)
                hdt_message = create_hdt_message(self.otter.current_orientation[2])
                rot_message = create_rot_message(self.otter.current_rotational_velocities[2])
            ...
            if self.latest_system_mode.standby_mode == True:
                ...
                self.fx, self.fz = self.map_rpm_to_force_moment(self.latest_system_mode.rpm_strb_test, self.latest_system_mode.rpm_port_test)
                self.otter.set_manual_control_mode(self.fx,0.0,self.fz)
