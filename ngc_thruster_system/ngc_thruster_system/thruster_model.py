...
class Thruster():
    def __init__(self,name:str,  thrust_torque_curve:str, prop_diameter):
        ...
        self.rps_upper = 0.0
        self.rps_lower = 0.0
        self.pitch_upper = 0.0
        self.pitch_lower = 0.0
    ...
    @abstractmethod
    def simulated_actuator_dynamics(self, nu, n, pitch, angle = None, dt = None):
        return NotImplementedError
...
class ControllablePitchPropellerAndRudder(Thruster):
    def __init__(self, name: str, thrust_torque_curve: str, prop_diameter):
      ...
      self.time_constant_rudder = 0.3
      self.time_constant_pitch = 0.1
      
    def get_tau(self, nu:np.array, n:float, pitch:float, angle:float = None, dt:float = None):
        """_summary_

        Args:
            nu (np.array): the relative velocity of the vessle
            n (float): Propeller revolutions per. second (works as setpoint during simulations and ta estimator during operations)
            pitch (float): Propeller pitch (works as setpoint during simulations and ta estimator during operations)
            angle (float, optional): Rudder angle (works as setpoint during simulations and ta estimator during operations)
            dt (float, optional): Simulation sample time, used to simulate actuator ramping delay. Defaults to None.

        Returns:
            tau(np.array): Propeller and rudder forces
        """
        if dt != None:
            n, pitch, angle = self.simulated_actuator_dynamics(n=n,pitch=pitch,angle=angle,dt=dt)
        ...
        f_alpha = (6.13*A_R)/(A_R +2.25) #rudder profile estimate beased on a series of datasheets in 
        t_R = 0.45-0-25*C_B
        x_H = -(0.4 +0.1*C_B)
        ...
        if n == 0.0: #if the propeller is at stand still
            ...
            du_inf = (8*tau_prop_x/(roh*math.pi*self.prop_diameter**2))**(1/2) #the slip stream velocity due to pressure increase over propeller
            r_inf = r_0 *(0.5*(1+(UR0/du_inf)))**(1/2) #the slip stream radius far away from the propeller (source Brix p.84)
            ...
            u_R = (eta * u_RP**2 +(1- eta) *UR0**2)**(1/2)

        else: #if the vessle is moving and the propeller is running
            u_R = epsilon *u*(1-wp) * (eta*(1+ k * ((1 + (8*self.thrust_torque_curve(pitch = pitch,J= J)/(math.pi *J**2)))**(0.5)-1)**2 +(1-eta)))**0.5 
    ...
    def simulated_actuator_dynamics(self, n, pitch, angle=None, dt=None):
        self.n += (n - self.n) * (dt / self.time_constant_n)
        self.pitch += (pitch - self.pitch) * (dt / self.time_constant_pitch)
        self.angle += (angle - self.angle) * (dt / self.time_constant_rudder)
        return self.n , self.pitch, self.angle
